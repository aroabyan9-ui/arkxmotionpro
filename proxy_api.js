...
// ── POST /proxy/openai/images ── forward ke OpenAI Images API
router.post('/openai/images', async (req, res) => {
  const apiKey = req.headers['x-openai-api-key'];
  if (!apiKey) return res.status(401).json({ error: 'Missing x-openai-api-key header' });

  try {
    const resp = await axios.post('https://api.openai.com/v1/images/generations', req.body, {
      headers: {
...
// ── GET /proxy/debug-proxy ── Debug proxy status
router.get('/debug-proxy', (req, res) => {
  const stats = proxyManager.getStats();
  const all = proxyManager.getAll();
  const envList = process.env.PROXY_LIST || 'NOT SET';
  res.json({
    stats,
    proxies: all.map(p => ({ id: p.id, url: p.url.replace(/:([^:@]+)@/, ':***@'), status: p.status })),
    env_proxy_list: envList.substring(0, 100) + (envList.length > 100 ? '...' : ''),
    env_count: envList !== 'NOT SET' ? envList.split(',').length : 0
  });
});
